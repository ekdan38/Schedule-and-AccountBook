...
public class P_CustomAdapter extends RecyclerView.Adapter<P_CustomAdapter.CustomViewHolder> {

    private ArrayList<P_User> arrayList;
    private Context context;

    public void updateData(ArrayList<P_User> newarrayList){
        arrayList = newarrayList;
        notifyDataSetChanged();
    }


    public P_CustomAdapter(ArrayList<P_User> list, Context context) {
        this.arrayList = list;
        this.context = context;
        Collections.sort(arrayList, new Comparator<P_User>() {
            @Override
            public int compare(P_User user1, P_User user2) {
    ...
    public CustomViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.p_list_item, parent,false);
    ...
    public void onBindViewHolder(CustomViewHolder holder, final int position) {
        ...
        P_User user = arrayList.get(position);
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                FragmentManager fragmentManager = ((AppCompatActivity) context).getSupportFragmentManager();
                P_EditTransactionDialog editTransactionDialog = new P_EditTransactionDialog();
                ...
                editTransactionDialog.show(fragmentManager, "p_EditTransactionDialog");
            }
        });
    }
